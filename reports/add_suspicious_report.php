...
try {
    ...
    if (isset($_FILES["images"])) {
        $upload_dir = __DIR__ . "/../uploads/suspicious_reports/";
        ...
        for ($i = 0; $i < count($_FILES["images"]["name"]); $i++) {
            if ($_FILES["images"]["error"][$i] === UPLOAD_ERR_OK) {
                ...
                $new_file_name = "suspicious_" . $report_id . "_" . time() . "_" . $i . "." . $file_extension;
                $target_path = $upload_dir . $new_file_name;

                if (move_uploaded_file($_FILES["images"]["tmp_name"][$i], $target_path)) {
                    $image_path = "uploads/suspicious_reports/" . $new_file_name;

                    $img_stmt = $conn->prepare("
                        INSERT INTO suspicious_report_image
    ...
    $conn,
    $user_id,
    "Your suspicious item report has been submitted and is waiting for admin approval.",
    "report_submitted",
    null
);
    ...
    echo json_encode(array(
        "status" => "success",
        "message" => "Suspicious report submitted successfully",
    ...
    echo json_encode(array(
        "status" => "error",
        "message" => "Failed to submit suspicious report",
        "error" => $e->getMessage()
    ));
}
?>