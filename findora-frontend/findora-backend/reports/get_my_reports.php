...
function getImages($conn, $table, $idColumn, $idValue) {
    $images = array();
...
/* Missing Pet Posts */
$missing_pet_posts = array();

$petStmt = $conn->prepare("
    SELECT *
    FROM missing_pet_post
    WHERE user_id = ?
    ORDER BY pet_post_id DESC
");
...
if ($petStmt) {
    ...
    while ($row = $petResult->fetch_assoc()) {
        ...
        $row["images"] = getImages(
            $conn,
            "missing_pet_post_image",
            "pet_post_id",
            intval($row["pet_post_id"])
        );

        $missing_pet_posts[] = $row;
    }
}

/* Missing Person Posts */
$missing_person_posts = array();

$personStmt = $conn->prepare("
    SELECT *
    FROM missing_person_post
    WHERE user_id = ?
    ORDER BY person_post_id DESC
");
...
if ($personStmt) {
    ...
    while ($row = $personResult->fetch_assoc()) {
        ...
        $row["images"] = getImages(
            $conn,
            "missing_person_post_image",
            "person_post_id",
            intval($row["person_post_id"])
        );

        $missing_person_posts[] = $row;
    }
}
...
echo json_encode(array(
    ...
    "missing_pet_posts" => $missing_pet_posts,
    "missing_person_posts" => $missing_person_posts
));
exit();
?>